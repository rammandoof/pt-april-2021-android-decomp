#macro COMMANDS_FILE_PATH "/storage/emulated/0/Documents/pizza tower april 2021 android/commands/"
//TODO: READ FROM AN EXTERNAL LANGUAGE FILE SO PEOPLE CAN MAKE CUSTOM LANGAUAGES FOR TEXT HERE.
// old editor stuff
global.door_object_arr = [obj_taxi, obj_startgate, obj_snickchallengedoor, obj_door, obj_keydoor, obj_geromedoor, obj_hallway]
...
function docommand(commandstring,silentcommand = false,is_trigger = false) 
{
	...
	if(string_pos("reload_gml", string_lower(commandstring)) == 1) 
	{
		...
		{
			var events = variable_struct_get_names(event) // OSVNA Moment????!??!?ß
	...
	if(string_pos("goto_room", string_lower(commandstring)) == 1)
	{
		var commands = ""
		try{
		...
		for (var i = 1; i < array_length(commands); i++)
		{
				if(i == 2)
				{
					r = asset_get_index(commands[i])
					if(!(room_exists(r)))
					{
						show_message_async("Non-existant Room! Aborting...")
						return;
					}
				}
				if(i == 3)
				{
					door = commands[i]
					if(!object_exists(asset_get_index("obj_door" + door)))
					{
						show_message_async("Invalid Door! Aborting...")
						return;
					}
				}
		}
	...
	if(string_pos("exec", string_lower(commandstring)) == 1) { // exec my goat
	...
	switch (string_lower(commandstring)) 
	{
		...
		case "showcollisions":
			...
			event_perform_object(obj_debugcontroller, ev_other, ev_room_start)
		break
		...
		break
		case "finalgame_parry true":
			global.finalgame_parry = true
		break
		case "finalgame_parry false":
			global.finalgame_parry = false
		break
		case "finalgame_parry":
		case "finalgame_parrytoggle":
		...
		break
		case "help":
			get_string_async("Available Commands: ","noclip, showcollisions <boolean>, panic <seconds>, hidetiles, showtiles, toggletiles, character <string>, character2 <string>, player_set_state <states.state>,instance_set_variable <obj_> <type> <variablename> <new value>,global_set_variable <type> <variablename> <new value>,spawn <obj_> <optional distance>, play_sound <sound>, play_music <sound>, togglebinds, gml <gml_code>, instance_set_variable_all <obj_> <type> <variablename> <new value>, reload_gml, heatmetertoggle, exec <filename>, savecommand <commands>, togglefps, goto_room <room_name> <door_name>, finalgame_parry")
		break
	}
}
